...
describe("GET /api-reservation/all-reservations/:page ", () => {
  test("It should respond with error since page value is not right",
  async () => {
    const response = await request(app)
    .get("/api-reservation/all-reservations/m")
...
describe("GET /api-reservation/all-reservations-by-date/"
  + ":page?from=val&to=val2",
  () => {
    test("Sending invalid date value; It should responce with error",
    async () => {
      const response = await request(app)
      .get("/api-reservation/all-reservations-by-date/"
        + "1?from=03/16/2022&to=12/18/202e")
      .set('Authorization', `Bearer ${token}`);
...
describe("GET /api-reservation/all-reservations-by-tables/:tables/:page",
  () => {
    test("Sending invalid tables value; It should responce with error",
    async () => {
      const response = await request(app)
      .get("/api-reservation/all-reservations-by-tables/1,a/1")
      .set('Authorization', `Bearer ${token}`);
